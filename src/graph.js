...
function initGraph(container, graphData, options = {}) {
  ...
  // Twinkle 별 노드 + 임시 엣지 (레이아웃 중에만 사용, layoutstop 후 제거)
  if (Array.isArray(options.twinkles)) {
    for (const t of options.twinkles) {
      if (!t.nearest_post) continue;
      elements.push({
        group: 'nodes',
        data: { id: 'twinkle-' + t.slug, slug: t.slug, title: t.title },
        classes: 'twinkle-star',
      });
      elements.push({
        group: 'edges',
        data: { id: 'twinkle-edge-' + t.slug, source: 'twinkle-' + t.slug, target: t.nearest_post, weight: 2.0, isTwinkleEdge: true },
      });
    }
  }
  ...
  const cy = cytoscape({
    ...
    style: [
      ...
      {
        selector: 'node.twinkle-star.dimmed',
        style: { 'opacity': 0.15 },
      },
      {
  ...
  cy.on('layoutstop', function () {
    cy.fit(undefined, 40);
    applyDepthEffect(cy);
    cy.edges('[?isTwinkleEdge]').remove();
    cy.nodes('.twinkle-star').lock();
  });
...
function setupHoverHighlight(cy, threshold, defaultHighlightNode) {
  ...
  cy.on('mouseover', 'node', function (evt) {
    ...
    if (evt.target.hasClass('twinkle-star')) return;
...
function applyDepthEffect(cy) {
  const nodes = cy.nodes().filter(n => !n.hasClass('supernode') && !n.hasClass('twinkle-star'));
